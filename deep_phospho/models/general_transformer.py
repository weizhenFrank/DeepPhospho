...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        src = src + self.dropout1(src2) # attention dropout
        src = self.norm1(src)
        if hasattr(self, "activation"):
            src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))  # intermediate feature dropout
        else:  # for backward compatibility
            src2 = self.linear2(self.dropout(F.relu(self.linear1(src))))
        src = src + self.dropout2(src2) # output dropout
        src = self.norm2(src)
        return src
