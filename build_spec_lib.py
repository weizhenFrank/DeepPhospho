...
HelpMSG = '''
This script have two usages and both of them will build a Spectronaut-compatible spectral library

Usage 1: python build_spec_lib.py build -i pathA -r pathB -o pathC
    this will read DeepPhospho predicted ion intensity (in JSON format) from pathA and RT (in txt format) from pathB, 
    then generated a library and output to pathC
...
Usage 2: python build_spec_lib.py merge -l pathA pathB ... -o pathO
    ...
    [notice] the first library will be used as the major one (i.e. spectra and RT in first library will be kept in priority)
...
if __name__ == '__main__':
    ...
    parser_merge.add_argument('-l', '--lib', metavar='path', type=str, nargs='*', required=True,
                              help='Paths of two or more library files')
    # output
    ...
    if args.action.lower() == 'build':
        ...
        all_libs = args.lib
        if len(all_libs) < 2:
            raise ValueError(f'To merge libraries, the number of input libraries should be more than or equal to 2')
