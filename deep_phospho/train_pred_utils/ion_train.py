...
from deep_phospho.model_utils.utils_functions import get_loss_func, show_params_status
...
def train_ion_model(configs=None, config_load_msgs=None, config_overwrite_msgs=None, termin_flag=None):
        ...
        for idx, (inputs, y) in enumerate(train_dataloader):
            ...
            if iteration % configs['TRAINING_HYPER_PARAM']['save_param_interval'] == 0 and iteration != 0 \
                ...
                if configs['TRAINING_HYPER_PARAM']['Bert_pretrain']:
                    ...
                    if use_holdout:
                        best_test_res, iteration_best, best_model = evaluation(configs, model, device, logger,
                                                                               tf_writer_train,
                                                                               tf_writer_test,
    ...
    if use_holdout:
        model = best_model
        model.to(device)
        evaluation(configs, model, device, logger, tf_writer_train, tf_writer_test,
...
def evaluation(configs, model, device, logger, tf_writer_train, tf_writer_test,
