...
def sn_lib_to_pred_input(lib_path, output_folder):
    ...
    with open(os.path.join(output_folder, f'{data_name}-Ion_PredInput.txt'), 'w') as f:
        f.write('sequence\n')
        for p in intprecs:
            f.write(p + '\n')

    lib['IntPep'].drop_duplicates().to_csv(os.path.join(output_folder, f'{data_name}-RT_PredInput.txt'), index=False)
...
def sn_results_to_pred_input(result_path, output_folder):
    ...
    result['IntPep'].drop_duplicates().to_csv(os.path.join(output_folder, f'{data_name}-RT_PredInput.txt'), index=False)
...
def mq_to_pred_input(result_path, output_folder, mq_version='1.5'):
    ...
    df['IntPep'].drop_duplicates().to_csv(os.path.join(output_folder, f'{data_name}-RT_PredInput.txt'), index=False)
